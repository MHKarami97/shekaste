    ...
    >
      <div className="mb-3 flex items-baseline justify-between gap-2">
        <h2 className="flex items-baseline gap-2 text-[13px] font-medium tracking-wide text-ink dark:text-night-ink">
          <span
            className="h-[7px] w-[7px] rotate-45 rounded-[1px] bg-gold/70"
            aria-hidden
          />
          {title}
        </h2>
...
export function TextInput(props: React.InputHTMLAttributes<HTMLInputElement>) {
  return (
    <input {...props} className={`${controlBase} ${props.className ?? ""}`} />
  );
}
...
export function Button({
  ...
  const styles =
    variant === "primary"
      ? "bg-ink text-paper hover:-translate-y-px hover:bg-ink/88 active:translate-y-0 active:scale-[0.98] dark:bg-night-ink dark:text-night dark:hover:bg-night-ink/88"
      : "border border-line/70 text-ink hover:-translate-y-px hover:border-tan hover:bg-tan/10 active:translate-y-0 active:scale-[0.98] dark:border-night-line dark:text-night-ink dark:hover:bg-night-ink/10";
  return (
    <button
      {...props}
